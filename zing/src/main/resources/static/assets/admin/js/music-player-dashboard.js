...
function loadTrack(track_index1) {
    getSongById(track_index1).then(result => {
            console.log()
            let song = result.data;
            clearInterval(updateTimer1);
            resetValues();
            curr_track1.src = song.url;
            curr_track1.load();
            curr_track1.play();

            if (track_art1) {
                track_art1.style.backgroundImage = "url(" + song.image + ")";
            }
            if (track_name1) {
                track_name1.textContent = song.title;
            }
        }
    ).catch(error=>{
        alert("false");
